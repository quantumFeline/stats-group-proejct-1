...
\begin{figure}[htp]
    ...
    \label{fig:Metrics by number of trajectories}
\end{figure}

From this graph we can see that the change in F1 tracks the performance change on the increased data volume most clearly.
Therefore, for the next graphs we will focus on it as our primary metric.

Both increasing trajectory length and the number of trajectories improves the algorithm performance; however, increasing trajectory length has diminishing returns, capping at around 20.
